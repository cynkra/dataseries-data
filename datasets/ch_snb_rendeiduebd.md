...
- **canonical**: no (alternate for Bond yields; `ch_snb_rendoblid` is canonical)
...
## What is special
This is the SNB spot-rate (zero-coupon yield) cube, distinct from the par-yield
`rendoblid`. It crosses two dimensions: a bond category and a maturity, giving a
full **term structure** rather than a single benchmark. The maturity dimension runs
1J through 10J plus 20J and 30J, so you can read an entire spot curve for one day.
...
Swiss-franc rate history including the negative-rate years. Kept as an alternate
because it overlaps the canonical `rendoblid` benchmark but offers a richer
category-by-maturity grid; `rendoblid` is the headline daily yield series.
...
- Spot (zero-coupon) rates here are not directly comparable to the par yields in
  `rendoblid`; keep the two concepts separate when charting.
