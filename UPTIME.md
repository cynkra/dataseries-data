...
_Last run: **2026-07-11T14:38:21Z**. Auto-generated by `R/uptime.R` (daily GitHub Action). Do not edit by hand._
...
- 🔴 **Run-through (upstream)** — 1 source(s) failed to fetch — **upstream did not deliver** (previous data kept). — **open incident: 3 day(s)**
...
| Run-through (upstream) | 83.3% | 84.6% | 3 d |
...
- `ch_adecco_sjmi` — Failed to perform HTTP request. Caused by error in `curl::curl_fetch_memory()`: ! Timeout was reached [www.stellenmarktmonitor.uzh.ch]: Failed to connect to www.stellenmarktmonitor.uzh.ch port 443 after 15002 ms: Timeout was reached **— escalated: 3 consecutive days, see `etl-outage` issue**
